<?php
    ...
    try
    {
        ...
        {
            $sql = 'update `dataacquisition` set `ProvinceCheck` = 1 where `InstitutionNumber` = "'.$_GET['cid'].'";';
        ...
        else if (isset($_POST["unpass"]) && isset($_GET["cid"]))
        {
            ...
            $sql = 'delete from `dataacquisition` where `InstitutionNumber` = "'.$_GET['cid'].'";';
            if (mysql_query($sql, $connect))
            {
                echo "<script>alert(\"操作成功！\");</script>";
                if (!mysql_query("insert into `notice` (`Author`, `Type`, `Title`, `Text`) values (\"系统\", \"2\", \"<b>[系统]</b>企业".$_GET['cid']."上报数据退回通知\", \"<blockquote style='text-align: justify; margin: 0px 0px 0px 40px; border: none; padding: 0px;'>企业：<b>".$res['CompanyName']."</b>上报的报表中存在不合理的地方，已经退回，请修改后重新上报，特此通知。</blockquote><blockquote style='text-align: right; margin: 0px 0px 0px 40px; border: none; padding: 0px;'>操作人：".$_SESSION['userName']."（省用户）</blockquote>\")", $connect))
                {
