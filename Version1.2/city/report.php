	...
	<div class="container">
		...
		<div class="content">
			<?php
				...
				$sql = "select `CityGovermentName`, `CompanyAddr`, `CompanyNumber`, `CompanyName`, `CollectionTime`, `CheckLevel` from `citygoverment`, `dataacquisition`, `company`, `surveyperiod` where `dataacquisition`.`institutionNumber` = `company`.`CompanyNumber` and `dataacquisition`.`SurveyPeriodID` = `surveyperiod`.`SurveyID` and `citygoverment`.`CityGovermentUsername` = '".$_SESSION['userName']."';";
			    
				$result = mysql_query($sql, $connect);
				while ($res = mysql_fetch_assoc($result))
				{
					if (strstr($res['CompanyAddr'], $res['CityGovermentName']))
					{
